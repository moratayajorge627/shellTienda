...
export default function DashboardPage() {
  ...
  // Datos simulados de dashboard con lógica realista
  const [metrics, setMetrics] = useState({
    todaySales: 5000.00,
    todaySalesCount: 42,
    todayGrossProfit: 2000.00, // Sales - COGS (3000)
    todayExpenses: 550.00, // 400 diarios + 150 otros
    todayNetResult: 1450.00, // 2000 - 550
    cashInRegister: 3850.00,
    
    // Inventario
    activeProducts: 148,
    lowStockProducts: 4,
    outOfStockProducts: 1,
    inventoryValue: 42800.00,

    // Finanzas
    accountsPayable: 5000.00,
    pendingLoans: 9500.00,
    monthlyExpenses: 4200.00,
    monthlyNetResult: 18450.00,
  });
    ...
    <div className="space-y-8">
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="text-xs text-slate-400 font-mono">Corte en tiempo real</span>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <CardContent className="p-6">
              ...
              <div className="mt-3">
                ...
                <div className="flex items-center justify-between text-xs text-slate-400 mt-1">
                  <span>{metrics.todaySalesCount} transacciones</span>
          ...
          {isAdmin && (
            <Card className="glass-card hover:border-indigo-500/40 bg-gradient-to-br from-indigo-950/40 to-slate-900">
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-semibold text-slate-300 uppercase tracking-wider">Resultado Neto (Utilidad)</span>
                  <div className="h-9 w-9 rounded-lg bg-indigo-600/20 text-indigo-400 flex items-center justify-center">
                    <Receipt className="h-5 w-5" />
                  </div>
                </div>
                <div className="mt-3">
                  <div className="text-2xl font-black text-indigo-300">{formatCurrency(metrics.todayNetResult)}</div>
                  <div className="text-xs text-indigo-200/70 mt-1">
                    Utilidad limpia restante
                  </div>
                </div>
              </CardContent>
            </Card>
          ...
          <CardContent className="space-y-4 pt-4">
            <div className="p-4 rounded-xl bg-slate-900/90 border border-slate-800 space-y-2">
              <div className="flex justify-between items-center text-sm">
                <span className="text-slate-400">Cuentas por Pagar Proveedores:</span>
            ...
            <div className="p-4 rounded-xl bg-indigo-950/30 border border-indigo-500/20 space-y-2">
              ...
              <div className="text-2xl font-black text-white">{formatCurrency(metrics.monthlyNetResult)}</div>
